...
from ...utils import buildFilePathFromEnv, meshNamesSanitized, selectObjectsInCollection
...
class BU_SHOT_PUBLISH(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not asset_col:
            self.report(
                {"ERROR"},
                f"Asset not found in the scene to publish: {shot_asset_type}: {shot_asset_name}",
            )
            return {"FINISHED"}
